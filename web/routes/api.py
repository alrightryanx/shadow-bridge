...
# ========== Worker Coordinator (Multi-Machine) ==========
...
def workers_list():
    ...
    from ..services.worker_coordinator import get_worker_coordinator
    coord = get_worker_coordinator()
    return jsonify({"success": True, "workers": coord.get_all_workers()})


@api_bp.route("/workers/online", methods=["GET"])
@require_bridge_auth
@rate_limit
@api_error_handler
def workers_online():
    ...
    from ..services.worker_coordinator import get_worker_coordinator
    coord = get_worker_coordinator()
    return jsonify({"success": True, "workers": coord.get_online_workers()})


@api_bp.route("/workers/capacity", methods=["GET"])
@require_bridge_auth
@rate_limit
@api_error_handler
def workers_capacity():
    """Get aggregate capacity across all workers."""
    from ..services.worker_coordinator import get_worker_coordinator
    coord = get_worker_coordinator()
    return jsonify({"success": True, **coord.get_capacity_summary()})


@api_bp.route("/workers/register", methods=["POST"])
@require_bridge_auth
@rate_limit
@api_error_handler
def worker_register():
...
def worker_status(worker_id):
    ...
    from ..services.worker_coordinator import get_worker_coordinator
    coord = get_worker_coordinator()
    status = coord.get_worker_status(worker_id)
    if not status:
        return jsonify({"success": False, "error": "Worker not found"}), 404
    return jsonify({"success": True, "worker": status})


@api_bp.route("/workers/<worker_id>/remove", methods=["POST"])
@require_bridge_auth
@rate_limit
@api_error_handler
def worker_remove(worker_id):
    """Remove a worker. Set drain=true to stop its agents first."""
    from ..services.worker_coordinator import get_worker_coordinator
    data = request.get_json(force=True) if request.is_json else {}
    coord = get_worker_coordinator()
    result = coord.remove_worker(worker_id, drain=bool(data.get("drain", False)))
    return jsonify(result)


@api_bp.route("/workers/<worker_id>/agents", methods=["GET"])
@require_bridge_auth
@rate_limit
@api_error_handler
def worker_agents(worker_id):
    """Get agents running on a specific worker."""
    from ..services.worker_coordinator import get_worker_coordinator
    coord = get_worker_coordinator()
    agents = [a for a in coord.get_all_remote_agents() if a.get("worker_id") == worker_id]
    return jsonify({"success": True, "agents": agents})


@api_bp.route("/workers/spawn", methods=["POST"])
@require_bridge_auth
@rate_limit
@api_error_handler
def worker_spawn_agent():
    """Spawn an agent on the best available worker."""
    from ..services.worker_coordinator import get_worker_coordinator
    data = request.get_json(force=True)
    coord = get_worker_coordinator()
    worker_id = data.pop("worker_id", None)
    if worker_id:
        result = coord.spawn_agent_on_worker(worker_id, data)
    else:
        result = coord.spawn_agent_on_best_worker(data)
    return jsonify(result)


@api_bp.route("/workers/agents", methods=["GET"])
@require_bridge_auth
@rate_limit
@api_error_handler
def workers_all_agents():
    """Get all remote agents across all workers."""
    from ..services.worker_coordinator import get_worker_coordinator
    coord = get_worker_coordinator()
    return jsonify({"success": True, "agents": coord.get_all_remote_agents()})


@api_bp.route("/mq/stats", methods=["GET"])
@require_bridge_auth
@rate_limit
@api_error_handler
def mq_stats():
    """Get distributed message queue statistics."""
    from ..services.message_bus import get_distributed_bus
    dbus = get_distributed_bus()
    return jsonify({"success": True, **dbus.get_stats()})


@api_bp.route("/mq/poll/<channel>", methods=["GET"])
@require_bridge_auth
@rate_limit
@api_error_handler
def mq_poll(channel):
    """Poll for pending messages on a channel (used by workers)."""
    from ..services.message_bus import get_distributed_bus
    limit = request.args.get("limit", 50, type=int)
    dbus = get_distributed_bus()
    messages = dbus.poll_messages(channel, limit=limit)
    return jsonify({"success": True, "messages": messages, "count": len(messages)})


@api_bp.route("/mq/send", methods=["POST"])
@require_bridge_auth
@rate_limit
@api_error_handler
def mq_send():
    """Send a message via the distributed message queue."""
    from ..services.message_bus import get_distributed_bus
    data = request.get_json(force=True)
    if not data.get("channel") or not data.get("sender"):
        return jsonify({"success": False, "error": "channel and sender required"}), 400
    dbus = get_distributed_bus()
    ok = dbus.send_distributed(
