...
class AgentStateStore:
    ...
    def _create_tables(self):
        ...
        conn.executescript("""
            ...
            -- Worker nodes (remote machines for distributed agent execution)
            ...
            -- Distributed message queue (SQLite MQ for cross-machine messaging)
            CREATE TABLE IF NOT EXISTS message_queue (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                message_id TEXT NOT NULL UNIQUE,
                channel TEXT NOT NULL,           -- target agent_id, topic name, or '__broadcast__'
                message_type TEXT NOT NULL,      -- task, status, control, data, broadcast
                ...
                created_at TEXT NOT NULL,
                delivered_at TEXT,
                expires_at TEXT                  -- auto-expire old messages
            );
            CREATE INDEX IF NOT EXISTS idx_mq_channel ON message_queue(channel, status);
    ...
    # ---- Workers (remote machines) ----

    def upsert_worker(self, worker_id: str, host: str, port: int,
                      capacity: int = 100, label: str = "") -> None:
        """Register or update a remote worker."""
        conn = self._get_conn()
        now = datetime.utcnow().isoformat()
        conn.execute(
            """INSERT INTO workers (worker_id, host, port, capacity, label, registered_at)
               VALUES (?, ?, ?, ?, ?, ?)
               ON CONFLICT(worker_id) DO UPDATE SET
                   host = excluded.host, port = excluded.port,
                   capacity = excluded.capacity, label = excluded.label""",
            (worker_id, host, port, capacity, label, now),
        )
        conn.commit()
    ...
    def get_all_workers(self) -> List[Dict]:
        ...
        conn = self._get_conn()
        rows = conn.execute("SELECT * FROM workers ORDER BY registered_at").fetchall()
        return [dict(r) for r in rows]
    ...
    # ---- Distributed Message Queue (SQLite MQ) ----

    def enqueue_message(self, message_id: str, channel: str, message_type: str,
        ...
        conn.execute(
            """INSERT OR IGNORE INTO message_queue
               (message_id, channel, message_type, sender, payload_json, priority,
                status, created_at, expires_at)
               VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?)""",
            (message_id, channel, message_type, sender, json.dumps(payload),
             priority, now.isoformat(), expires),
        )
        conn.commit()

    def dequeue_messages(self, channel: str, limit: int = 50) -> List[Dict]:
        """Fetch and mark pending messages for a channel as delivered."""
        conn = self._get_conn()
        now = datetime.utcnow().isoformat()

        # Expire old messages first
        conn.execute(
            "UPDATE message_queue SET status = 'expired' WHERE expires_at IS NOT NULL AND expires_at < ? AND status = 'pending'",
            (now,),
        )
        ...
        if rows:
            ...
            conn.execute(
                f"UPDATE message_queue SET status = 'delivered', delivered_at = ? WHERE message_id IN ({placeholders})",
                [now] + ids,
            )
            conn.commit()
    ...
    def get_pending_message_count(self, channel: str = None) -> int:
        """Count pending messages, optionally filtered by channel."""
        conn = self._get_conn()
        if channel:
            row = conn.execute(
                "SELECT COUNT(*) as c FROM message_queue WHERE status = 'pending' AND channel = ?",
                (channel,),
            ).fetchone()
        else:
            row = conn.execute(
                "SELECT COUNT(*) as c FROM message_queue WHERE status = 'pending'"
            ).fetchone()
        return row["c"] if row else 0

    def cleanup_delivered_messages(self, older_than_hours: int = 24) -> int:
        """Remove delivered/expired messages older than N hours."""
        conn = self._get_conn()
        cutoff = (datetime.utcnow() - timedelta(hours=older_than_hours)).isoformat()
        cursor = conn.execute(
            "DELETE FROM message_queue WHERE status IN ('delivered', 'expired') AND created_at < ?",
            (cutoff,),
        )
        conn.commit()
        return cursor.rowcount
